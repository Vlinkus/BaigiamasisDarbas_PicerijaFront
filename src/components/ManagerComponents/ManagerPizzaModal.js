...
function ManagerPizzaModal({ showModal, closeModal, pizza }) {
    ...
    const pizzaModalClassName = showModal ? "modal display-block" : "modal display-none";
    ...
    const handleProductCheck = (selectedProduct, isChecked) => {
      if (isChecked) {
        ...
        setPizzaToEdit((prevPizza) => ({
          ...prevPizza,
          products: prevPizza.products.filter((product) => product.id !== selectedProduct.id),
        }));
  ...
  const imageSave = (id, formData) => {
    ...
    axios.post(pizzaPhotoUploadUrl, formData)
    .then((response) => {
  ...
  return (
    <>
      <div className={pizzaModalClassName} id="pizzaModal" tabIndex="-1" aria-labelledby="pizzaModalLabel" aria-hidden={!showModal}>
        <div className="modal-dialog modal-xl">
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title" id="pizzaModalLabel">Picos {pizzaToEdit.pizzaName ? 'Atnaujinimas' : 'Pridėjimas'}</h5>
            </div>
                ...
                <div className="row">
                  <div className="col">
                    <FormGroup>
                      ...
                      <Input type="text" name="pizzaName" id="pizzaName" value={pizzaToEdit.pizzaName || ''}
                        onChange={handleChange} autoComplete="name" />
                    </FormGroup>
                    <FormGroup>
                      <Label htmlFor="pizzaPrice">Kaina</Label>
                      <Input type="text" name="pizzaPrice" id="pizzaPrice" value={pizzaToEdit.pizzaPrice || ''}
                        onChange={handleChange} autoComplete="pizzaPrice" />
                    </FormGroup>
                    <FormGroup>
                      <Label htmlFor="pizzaSize">Dydis</Label>
                      <Input type="text" name="pizzaSize" id="pizzaSize" value={pizzaToEdit.pizzaSize || ''}
                        onChange={handleChange} autoComplete="pizzaSize" />
                    </FormGroup>
                    <FormGroup>
                      ...
                      {productsList.map(product => (
                        <div className="form-check" key={product.id}>
                          <Input className="form-check-input" type="checkbox"
                            ...
                            checked={pizzaToEdit.products.some(p => p.id === (product.id))}
                            onChange={(event) => {
                              const { value, checked } = event.target;
                              console.log("Is product Included in pizza ", pizzaToEdit.products.some(p => p.id === (product.id)));
                              handleProductCheck(product, checked);
                            }}
                          />
                          <Label className="form-check-label" htmlFor={`productCheckBox_${product.id}`}>
                            {product.productName}
                          </Label>
                        </div>
                      )
                  ...
                  <div className="col">
                    <div className="current-pizza-img">
                      <h4>{pizzaToEdit ? " Picos " + pizzaToEdit.pizzaName + " paveikslėlis" : "Picos paveikslo dar nėra"}</h4>
                      {pizzaToEdit.pizzaPhoto && (
                        <img src={base64ToImageUrl(pizzaToEdit.pizzaPhoto)} alt={"Picos paveikslėlis"} />
                      )}
                      <Label className="form-label" htmlFor="customFile">Pasirinkite naują paveikslėlį</Label>
                      <Input type="file" className="form-control" id="pizzaPhoto" onChange={handleImageUpload} />
                    </div>
                  </div>
                </div>
              </Form>
